...
export default function punctuateSeries(items) {
  if (items.length === 2) {
    return app.translator.trans('talk.lib.series.two_text', {
      first: items[0],
      second: items[1],
    });
    ...
    const second = items
      ...
      .reduce((list, item) => list.concat([item, app.translator.trans('talk.lib.series.glue_text')]), [])
      .slice(0, -1);

    return app.translator.trans('talk.lib.series.three_text', {
      first: items[0],
      second,
