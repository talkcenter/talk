...
export default class EditUserModal<CustomAttrs extends IEditUserModalAttrs = IEditUserModalAttrs> extends Modal<CustomAttrs> {
  ...
  title() {
    return app.translator.trans('talk.lib.edit_user.title');
  }
  ...
  content() {
    ...
    return (
      <div className="Modal-body">
        {fields.length > 1 ? <div className="Form">{this.fields().toArray()}</div> : app.translator.trans('talk.lib.edit_user.nothing_available')}
      </div>
    );
  }
    ...
    if (this.attrs.user.canEditCredentials()) {
      items.add(
        ...
        <div className="Form-group">
          <label>{app.translator.trans('talk.lib.edit_user.username_heading')}</label>
          <input
            className="FormControl"
            placeholder={extractText(app.translator.trans('talk.lib.edit_user.username_label'))}
            bidi={this.username}
      ...
      if (app.session.user !== this.attrs.user) {
        items.add(
          'email',
          <div className="Form-group">
            <label>{app.translator.trans('talk.lib.edit_user.email_heading')}</label>
            <div>
              <input
                className="FormControl"
                placeholder={extractText(app.translator.trans('talk.lib.edit_user.email_label'))}
                bidi={this.email}
                disabled={this.nonAdminEditingAdmin()}
              />
            </div>
            {!this.isEmailConfirmed() && this.userIsAdmin(app.session.user) && (
              <div>
                <Button className="Button Button--block" loading={this.loading} onclick={this.activate.bind(this)}>
                  {app.translator.trans('talk.lib.edit_user.activate_button')}
                </Button>
              </div>
            )}
          </div>,
        ...
        items.add(
          'password',
          <div className="Form-group">
            <label>{app.translator.trans('talk.lib.edit_user.password_heading')}</label>
            <div>
              <label className="checkbox">
                <input
                  ...
                  disabled={this.nonAdminEditingAdmin()}
                />
                {app.translator.trans('talk.lib.edit_user.set_password_label')}
              </label>
              {this.setPassword() && (
                <input
                  className="FormControl"
                  type="password"
                  name="password"
                  placeholder={extractText(app.translator.trans('talk.lib.edit_user.password_label'))}
                  bidi={this.password}
                  disabled={this.nonAdminEditingAdmin()}
                />
              )}
    ...
    if (this.attrs.user.canEditGroups()) {
      items.add(
        ...
        <div className="Form-group EditUserModal-groups">
          <label>{app.translator.trans('talk.lib.edit_user.groups_heading')}</label>
          <div>
    ...
    items.add(
      ...
      <div className="Form-group">
        <Button className="Button Button--primary" type="submit" loading={this.loading}>
          {app.translator.trans('talk.lib.edit_user.submit_button')}
        </Button>
      </div>,
      -10
    );
