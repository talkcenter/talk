...
export default class EditGroupModal<CustomAttrs extends IEditGroupModalAttrs = IEditGroupModalAttrs> extends Modal<CustomAttrs> {
  ...
  title() {
    return (
      <>
        ...
        {this.namePlural() || app.translator.trans('talk.admin.edit_group.title')}
      </>
    );
  }
  ...
  fields(): ItemList<Mithril.Children> {
    ...
    items.add(
      ...
      <div className="Form-group">
        <label>{app.translator.trans('talk.admin.edit_group.name_label')}</label>
        <div className="EditGroupModal-name-input">
          <input className="FormControl" placeholder={app.translator.trans('talk.admin.edit_group.singular_placeholder')} bidi={this.nameSingular} />
          <input className="FormControl" placeholder={app.translator.trans('talk.admin.edit_group.plural_placeholder')} bidi={this.namePlural} />
        </div>
      </div>,
      30
    );
    ...
    items.add(
      'color',
      <div className="Form-group">
        <label>{app.translator.trans('talk.admin.edit_group.color_label')}</label>
    ...
    items.add(
      'icon',
      <div className="Form-group">
        <label>{app.translator.trans('talk.admin.edit_group.icon_label')}</label>
        <div className="helpText">
          {app.translator.trans('talk.admin.edit_group.icon_text', { a: <a href="https://fontawesome.com/v5/search?m=free" tabindex="-1" target="_blank" /> })}
        </div>
    ...
    items.add(
      ...
      <div className="Form-group">
        <Switch state={this.isHidden()} onchange={this.isHidden}>
          {app.translator.trans('talk.admin.edit_group.hide_label')}
        </Switch>
      </div>,
      10
    );
    ...
    items.add(
      ...
      <div className="Form-group">
        <Button type="submit" className="Button Button--primary EditGroupModal-save" loading={this.loading}>
          {app.translator.trans('talk.admin.edit_group.submit_button')}
        </Button>

        {this.group.exists && this.group.id() !== Group.ADMINISTRATOR_ID && (
          <button type="button" className="Button EditGroupModal-delete" onclick={this.deleteGroup.bind(this)}>
            {app.translator.trans('talk.admin.edit_group.delete_button')}
          </button>
        )}
      </div>,
      -10
  ...
  deleteGroup() {
    if (confirm(extractText(app.translator.trans('talk.admin.edit_group.delete_confirmation')))) {
