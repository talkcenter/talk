...
export default class ExtensionPage<Attrs extends ExtensionPageAttrs = ExtensionPageAttrs> extends AdminPage<Attrs> {
      ...
      <div className={'ExtensionPage ' + this.className()}>
        ...
        {!this.isEnabled() ? (
          <div className="container">
            <h3 className="ExtensionPage-subHeader">{app.translator.trans('talk.admin.extension.enable_to_see')}</h3>
          </div>
        ) : (
        ...
        <div className="container">
          <div className="ExtensionTitle">
            ...
            <div className="ExtensionName">
              <h2>{this.extension.extra['talk-extension'].title}</h2>
            </div>
          ...
          <div className="ExtensionPage-headerItems">
            ...
            >
              {isEnabled ? app.translator.trans('talk.admin.extension.enabled') : app.translator.trans('talk.admin.extension.disabled')}
            </Switch>
    ...
    items.add(
      ...
      <div className="ExtensionPage-permissions">
        <div className="ExtensionPage-permissions-header">
          <div className="container">
            <h2 className="ExtensionTitle">{app.translator.trans('talk.admin.extension.permissions_title')}</h2>
          </div>
        </div>
        <div className="container">
          {app.extensionData.extensionHasPermissions(this.extension.id) ? (
            ...
            <h3 className="ExtensionPage-subHeader">{app.translator.trans('talk.admin.extension.no_permissions')}</h3>
          )}
        </div>
      </div>
    );
      ...
      <div className="ExtensionPage-settings">
        <div className="container">
          {settings ? (
            ...
            <h3 className="ExtensionPage-subHeader">{app.translator.trans('talk.admin.extension.no_settings')}</h3>
          )}
        </div>
      </div>
    );
  ...
  topItems() {
    ...
    if (!this.isEnabled()) {
      const purge = () => {
        if (confirm(extractText(app.translator.trans('talk.admin.extension.confirm_purge')))) {
          app
            .request({
      ...
      items.add(
        ...
        <Button icon="fas fa-trash-alt" className="Button Button--primary" onclick={purge.bind(this)}>
          {app.translator.trans('talk.admin.extension.purge_button')}
        </Button>
      );
    }
  ...
  infoItems() {
      ...
      if (links[field]) {
        items.add(
          ...
          <LinkButton href={links[field]} icon={this.infoFields[field]} external={true} target="_blank">
            {app.translator.trans(`talk.admin.extension.info_links.${field}`)}
          </LinkButton>
        );
      }
    });
    ...
    items.add(
      ...
      <Button icon="fab fa-readme" className="Button Button--text" onclick={() => app.modal.show(ReadmeModal, { extension })}>
        {app.translator.trans('talk.admin.extension.readme.button_label')}
      </Button>,
      10
    );
  ...
  onerror(e: RequestError) {
    ...
    if (error) {
      app.alerts.show(
        { type: 'error' },
        app.translator.trans(`talk.lib.error.${error.code}_message`, {
