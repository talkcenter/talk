  ...
  extensionCategory(category) {
    return (
      <div className="ExtensionList-Category">
        <h4 className="ExtensionList-Label">{app.translator.trans(`talk.admin.nav.categories.${category}`)}</h4>
      ...
      <li className={classList('ExtensionListItem', { disabled: !isExtensionEnabled(extension.id) })}>
        <Link href={app.route('extension', { id: extension.id })}>
          <div className="ExtensionListItem-content">
            ...
            <span className="ExtensionListItem-title">{extension.extra['talk-extension'].title}</span>
          </div>
        </Link>
      </li>
    );
