...
export default class CreateUserModal<CustomAttrs extends ICreateUserModalAttrs = ICreateUserModalAttrs> extends Modal<CustomAttrs> {
  ...
  title() {
    return app.translator.trans('talk.admin.create_user.title');
  }
  ...
  fields() {
    ...
    const usernameLabel = extractText(app.translator.trans('talk.admin.create_user.username_placeholder'));
    const emailLabel = extractText(app.translator.trans('talk.admin.create_user.email_placeholder'));
    const emailConfirmationLabel = extractText(app.translator.trans('talk.admin.create_user.email_confirmed_label'));
    const useRandomPasswordLabel = extractText(app.translator.trans('talk.admin.create_user.use_random_password'));
    const passwordLabel = extractText(app.translator.trans('talk.admin.create_user.password_placeholder'));
    ...
    items.add(
      ...
      <div className="Form-group">
        <Button className="Button Button--primary Button--block" type="submit" loading={this.loading}>
          {app.translator.trans('talk.admin.create_user.submit_button')}
        </Button>
      </div>,
      0
    );
    ...
    items.add(
      ...
      <div className="Form-group">
        <Button className="Button Button--block" onclick={() => this.bulkAdd(true) && this.onsubmit()} disabled={this.loading}>
          {app.translator.trans('talk.admin.create_user.submit_and_create_another_button')}
        </Button>
      </div>,
      -20
    );
