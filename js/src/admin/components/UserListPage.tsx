...
export default class UserListPage extends AdminPage {
   ...
   * data provided by `AdminPayload.php`, or `talk/statistics`
   * if installed.
   */
    ...
    return [
          ...
          columns.map((col, colIndex) => {
            ...
            return (
              <div
                ...
                role="cell"
              >
                {columnContent || app.translator.trans('talk.admin.users.grid.invalid_column_content')}
              </div>
            );
          })
        )}
      ...
      <nav className="UserListPage-gridPagination">
        <Button
          ...
          title={app.translator.trans('talk.admin.users.pagination.first_page_button')}
        ...
        <Button
          disabled={this.pageNumber === 0}
          title={app.translator.trans('talk.admin.users.pagination.back_button')}
          onclick={this.previousPage.bind(this)}
          icon="fas fa-chevron-left"
          className="Button Button--icon UserListPage-backBtn"
        />
        <span className="UserListPage-pageNumber">
          {app.translator.trans('talk.admin.users.pagination.page_counter', {
            ...
            current: (
              <input
                ...
                aria-label={extractText(app.translator.trans('talk.admin.users.pagination.go_to_page_textbox_a11y_label'))}
                autocomplete="off"
        ...
        <Button
          ...
          title={app.translator.trans('talk.admin.users.pagination.next_button')}
        ...
        <Button
          disabled={!this.moreData}
          title={app.translator.trans('talk.admin.users.pagination.last_page_button')}
  ...
  headerItems(): ItemList<Mithril.Children> {
    ...
    items.add(
      ...
      <div className="Search-input">
        <input
          ...
          type="search"
          placeholder={app.translator.trans('talk.admin.users.search_placeholder')}
    ...
    items.add(
      'totalUsers',
      <p class="UserListPage-totalUsers">{app.translator.trans('talk.admin.users.total_users', { count: this.userCount })}</p>,
      90
    );
  ...
  actionItems(): ItemList<Mithril.Children> {
    ...
    items.add(
      ...
      <Button className="Button UserListPage-createUserBtn" icon="fas fa-user-plus" onclick={() => app.modal.show(CreateUserModal)}>
        {app.translator.trans('talk.admin.users.create_user_button')}
      </Button>,
      100
    );
  ...
  columns(): ItemList<ColumnData> {
    ...
    columns.add(
      'id',
      {
        name: app.translator.trans('talk.admin.users.grid.columns.user_id.title'),
    ...
    columns.add(
      'username',
      {
        name: app.translator.trans('talk.admin.users.grid.columns.username.title'),
        content: (user: User) => {
          ...
          return (
            <a
              target="_blank"
              href={profileUrl}
              title={extractText(app.translator.trans('talk.admin.users.grid.columns.username.profile_link_tooltip', { username: user.username() }))}
            >
              {user.username()}
            </a>
          );
    ...
    columns.add(
      'displayName',
      {
        name: app.translator.trans('talk.admin.users.grid.columns.display_name.title'),
    ...
    columns.add(
      'joinDate',
      {
        name: app.translator.trans('talk.admin.users.grid.columns.join_time.title'),
        content: (user: User) => (
    ...
    columns.add(
      'groupBadges',
      {
        name: app.translator.trans('talk.admin.users.grid.columns.group_badges.title'),
        content: (user: User) => {
          ...
          if (badges.length) {
            ...
            return app.translator.trans('talk.admin.users.grid.columns.group_badges.no_badges');
          }
        },
      },
      70
    ...
    columns.add(
      'emailAddress',
      {
        name: app.translator.trans('talk.admin.users.grid.columns.email.title'),
        content: (user: User) => {
          function setEmailVisibility(visible: boolean) {
            ...
            emailToggleButton.attr(
              ...
              extractText(
                visible
                  ? app.translator.trans('talk.admin.users.grid.columns.email.visibility_hide')
                  : app.translator.trans('talk.admin.users.grid.columns.email.visibility_show')
              )
            );
          ...
          return (
            <div className="UserList-email" key={user.id()} data-email-shown="false">
              ...
              <button
                ...
                title={app.translator.trans('talk.admin.users.grid.columns.email.visibility_show')}
              >
    ...
    columns.add(
      'editUser',
      {
        name: app.translator.trans('talk.admin.users.grid.columns.edit_user.title'),
        content: (user: User) => (
          <Button
            className="Button UserList-editModalBtn"
            title={app.translator.trans('talk.admin.users.grid.columns.edit_user.tooltip', { username: user.username() })}
            onclick={() => app.modal.show(EditUserModal, { user })}
          >
            {app.translator.trans('talk.admin.users.grid.columns.edit_user.button')}
          </Button>
        ),
      },
      -90
  ...
  headerInfo() {
    return {
      ...
      icon: 'fas fa-users',
      title: app.translator.trans('talk.admin.users.title'),
      description: app.translator.trans('talk.admin.users.description'),
    };
  }
