...
export default class EditPostComposer extends ComposerBody {
  static initAttrs(attrs) {
    ...
    attrs.submitLabel = attrs.submitLabel || app.translator.trans('talk.site.composer_edit.submit_button');
    attrs.confirmExit = attrs.confirmExit || app.translator.trans('talk.site.composer_edit.discard_confirmation');
    ...
    items.add(
      ...
      <h3>
        ...
        <Link href={app.route.discussion(post.discussion(), post.number())} onclick={minimizeComposerIfFullScreen}>
          {app.translator.trans('talk.site.composer_edit.post_link', { number: post.number(), discussion: post.discussion().title() })}
        </Link>
      </h3>
    );
        ...
        const alert = app.alerts.show(
          {
            ...
            controls: [
              ...
              >
                {app.translator.trans('talk.site.composer_edit.view_button')}
              </Button>,
            ],
          },
          app.translator.trans('talk.site.composer_edit.edited_message')
        );
      }
