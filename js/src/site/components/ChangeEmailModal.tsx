...
export default class ChangeEmailModal<CustomAttrs extends IInternalModalAttrs = IInternalModalAttrs> extends Modal<CustomAttrs> {
  ...
  title() {
    return app.translator.trans('talk.site.change_email.title');
  }
    ...
    if (this.success) {
      items.add(
        ...
        <p className="helpText">
          {app.translator.trans('talk.site.change_email.confirmation_message', {
      ...
      items.add(
        ...
        <div className="Form-group">
          <Button className="Button Button--primary Button--block" onclick={this.hide.bind(this)}>
            {app.translator.trans('talk.site.change_email.dismiss_button')}
          </Button>
        </div>
      );
    } else {
      ...
      items.add(
        ...
        <div className="Form-group">
          <input
            ...
            placeholder={app.translator.trans('talk.site.change_email.confirm_password_placeholder')}
            bidi={this.password}
            disabled={this.loading}
          />
        </div>
      ...
      items.add(
        ...
        <div className="Form-group">
          <Button className="Button Button--primary Button--block" type="submit" loading={this.loading}>
            {app.translator.trans('talk.site.change_email.submit_button')}
          </Button>
        </div>
      );
    }
  ...
  onerror(error: RequestError) {
    if (error.status === 401 && error.alert) {
      error.alert.content = app.translator.trans('talk.site.change_email.incorrect_password_message');
    }
