...
export default class LogInModal<CustomAttrs extends ILoginModalAttrs = ILoginModalAttrs> extends Modal<CustomAttrs> {
  ...
  title() {
    return app.translator.trans('talk.site.log_in.title');
  }
  ...
  fields() {
    ...
    const identificationLabel = extractText(app.translator.trans('talk.site.log_in.username_or_email_placeholder'));
    const passwordLabel = extractText(app.translator.trans('talk.site.log_in.password_placeholder'));
      ...
      <div className="Form-group">
        <div>
          <label className="checkbox">
            ...
            {app.translator.trans('talk.site.log_in.remember_me_label')}
          </label>
        </div>
      </div>,
      10
    ...
    items.add(
      ...
      <div className="Form-group">
        <Button className="Button Button--primary Button--block" type="submit" loading={this.loading}>
          {app.translator.trans('talk.site.log_in.submit_button')}
        </Button>
      </div>,
      -10
    );
    ...
    return (
      <>
        <p className="LogInModal-forgotPassword">
          <a onclick={this.forgotPassword.bind(this)}>{app.translator.trans('talk.site.log_in.forgot_password_link')}</a>
        </p>
        {app.site.attribute<boolean>('allowSignUp') && (
          <p className="LogInModal-signUp">{app.translator.trans('talk.site.log_in.sign_up_text', { a: <a onclick={this.signUp.bind(this)} /> })}</p>
        )}
      </>
    );
  }
  ...
  onerror(error: RequestError) {
    if (error.status === 401 && error.alert) {
      error.alert.content = app.translator.trans('talk.site.log_in.invalid_login_message');
      this.password('');
    }
