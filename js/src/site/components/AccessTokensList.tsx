...
export default class AccessTokensList<CustomAttrs extends IAccessTokensListAttrs = IAccessTokensListAttrs> extends Component<CustomAttrs> {
    ...
    return (
      <div className="AccessTokensList">
        {this.attrs.tokens.length ? (
          ...
          <div className="AccessTokensList--empty">{app.translator.trans('talk.site.security.empty_text')}</div>
        )}
      </div>
    );
  }
  ...
  tokenInfoItems(token: AccessToken) {
    ...
    if (this.attrs.type === 'session') {
      items.add(
        ...
        <div className="AccessTokensList-item-title">
          ...
          {token.isCurrent() && [
            ' — ',
            <span className="AccessTokensList-item-title-sub">{app.translator.trans('talk.site.security.current_active_session')}</span>,
          ]}
        </div>
      );
    } else {
    ...
    items.add(
      ...
      <div className="AccessTokensList-item-createdAt">
        <LabelValue label={app.translator.trans('talk.site.security.created')} value={humanTime(token.createdAt())} />
      </div>
    );
    ...
    items.add(
      ...
      <div className="AccessTokensList-item-lastActivityAt">
        <LabelValue
          label={app.translator.trans('talk.site.security.last_activity')}
          value={
            token.lastActivityAt() ? (
              ...
              app.translator.trans('talk.site.security.never')
            )
          }
        />
      </div>
  ...
  tokenActionItems(token: AccessToken) {
    ...
    if (this.attrs.type === 'developer_token') {
      ...
      items.add(
        ...
        >
          {app.translator.trans(`talk.site.security.${displayKey}`)}
        </Button>
      );
    }
    ...
    let revokeButton = (
      <Button className="Button Button--danger" disabled={token.isCurrent()} loading={!!this.loading[token.id()!]} onclick={() => this.revoke(token)}>
        {app.translator.trans(`talk.site.security.${deleteKey}`)}
      </Button>
    );
  ...
  async revoke(token: AccessToken) {
    if (!confirm(extractText(app.translator.trans('talk.site.security.revoke_access_token_confirmation')))) return;
    ...
    app.alerts.show({ type: 'success' }, app.translator.trans(`talk.site.security.${key}`, { count: 1 }));
    m.redraw();
  }

  generateTokenTitle(token: AccessToken): NestedStringArray {
    const name = token.title() || app.translator.trans('talk.site.security.token_title_placeholder');
    const value = this.tokenValueDisplay(token);

    return app.translator.trans('talk.site.security.token_item_title', { title: name, token: value });
  }
