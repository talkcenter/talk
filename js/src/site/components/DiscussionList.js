...
export default class DiscussionList extends Component {
  view() {
    ...
    if (isLoading) {
      ...
      loading = (
        <Button className="Button" onclick={state.loadNext.bind(state)}>
          {app.translator.trans('talk.site.discussion_list.load_more_button')}
        </Button>
      );
    }

    if (state.isEmpty()) {
      const text = app.translator.trans('talk.site.discussion_list.empty_text');
      return (
    ...
    return (
      <div className={classList('DiscussionList tc-discussion-list', { 'DiscussionList--searchResults': state.isSearchResults() })}>
