...
export default class UserPage<CustomAttrs extends IUserPageAttrs = IUserPageAttrs, CustomState = undefined> extends Page<CustomAttrs, CustomState> {
  ...
  navItems() {
    ...
    items.add(
      'posts',
      <LinkButton href={app.route('user.posts', { username: user.slug() })} icon="fa fa-chart-line">
        {app.translator.trans('talk.site.user.posts_link')} <span className="Button-badge">{user.commentCount()}</span>
      </LinkButton>,
      100
    );
    ...
    items.add(
      ...
      <LinkButton href={app.route('user.discussions', { username: user.slug() })} icon="fas fa-bars">
        {app.translator.trans('talk.site.user.discussions_link')} <span className="Button-badge">{user.discussionCount()}</span>
      </LinkButton>,
      90
    );
    ...
    if (isActor) {
      ...
      items.add(
        ...
        <LinkButton href={app.route('settings')} icon="fas fa-cog">
          {app.translator.trans('talk.site.user.settings_link')}
        </LinkButton>,
        -100
      );
    }
    ...
    if (isActor || app.site.attribute<boolean>('canModerateAccessTokens')) {
      ...
      items.add(
        ...
        <LinkButton href={app.route('user.security', { username: user.slug() })} icon="fas fa-shield-alt">
          {app.translator.trans('talk.site.user.security_link')}
        </LinkButton>,
        -100
      );
    }
