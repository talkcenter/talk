...
export default class IndexPage<CustomAttrs extends IIndexPageAttrs = IIndexPageAttrs, CustomState = {}> extends Page<CustomAttrs, CustomState> {
  ...
  oninit(vnode: Mithril.Vnode<CustomAttrs, this>) {
    ...
    app.history.push('index', extractText(app.translator.trans('talk.site.header.back_to_index_tooltip')));
    ...
    return (
      <div className="IndexPage">
        ...
        <div className="container">
          <div className="sideNavContainer tc-indexpage">

            <div className="IndexPage-results sideNavOffset tc-indexpage-body">
              <div className="IndexPage-toolbar tc-indexpage-toolbar">
            ...
            <side className="IndexPage-nav sideNav tc-indexpage-nav">
              <div>{listItems(this.sidebarItems().toArray())}</div>
            </side>
          </div>
        </div>
      </div>
  ...
  setTitle() {
    app.setTitle(extractText(app.translator.trans('talk.site.index.meta_title_text')));
    app.setTitleCount(0);
  }
  ...
  sidebarItems() {
    ...
    items.add(
      'newDiscussion tc-new-discussion',
      <Button
        icon="fas fa-edit"
      ...
      >
        {app.translator.trans(`talk.site.index.${canStartDiscussion ? 'start_discussion_button' : 'cannot_start_discussion_button'}`)}
      </Button>
    );
    ...
    items.add(
      ...
      <SelectDropdown
        ...
        accessibleToggleLabel={app.translator.trans('talk.site.index.toggle_sidenav_dropdown_accessible_label')}
      >
  ...
  navItems() {
    ...
    items.add(
      ...
      <LinkButton href={app.route('index', params)} icon="far fa-comments">
        {app.translator.trans('talk.site.index.all_discussions_link')}
      </LinkButton>,
      100
    );
  ...
  viewItems() {
    ...
    const sortOptions = Object.keys(sortMap).reduce((acc: any, sortId) => {
      acc[sortId] = app.translator.trans(`talk.site.index_sort.${sortId}_button`);
      return acc;
    }, {});
    ...
    items.add(
      ...
      <Dropdown
        ...
        accessibleToggleLabel={app.translator.trans('talk.site.index_sort.toggle_dropdown_accessible_label')}
      >
  ...
  actionItems() {
    ...
    items.add(
      'refresh',
      <Button
        title={app.translator.trans('talk.site.index.refresh_tooltip')}
        icon="fas fa-sync"
    ...
    if (app.session.user) {
      items.add(
        'markAllAsRead',
        <Button
          title={app.translator.trans('talk.site.index.mark_all_as_read_tooltip')}
          icon="fas fa-check"
          className="Button Button--icon"
   ...
   */
  markAllAsRead() {
    const confirmation = confirm(extractText(app.translator.trans('talk.site.index.mark_all_as_read_confirmation')));
