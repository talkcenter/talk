...
export default class UserSecurityPage<CustomAttrs extends IUserPageAttrs = IUserPageAttrs> extends UserPage<CustomAttrs, UserSecurityPageState> {
  ...
  oninit(vnode: Mithril.Vnode<CustomAttrs, this>) {
    ...
    app.setTitle(extractText(app.translator.trans('talk.site.security.title')));
  ...
  settingsItems() {
    ...
    if (
      ...
      items.add(
        'developerTokens',
        <FieldSet className="UserSecurityPage-developerTokens" label={app.translator.trans(`talk.site.security.developer_tokens_heading`)}>
    ...
    items.add(
      'sessions',
      <FieldSet className="UserSecurityPage-sessions" label={app.translator.trans(`talk.site.security.sessions_heading`)}>
    ...
    if (this.user!.id() === app.session.user!.id()) {
      items.add(
        'globalLogout',
        <FieldSet className="UserSecurityPage-globalLogout" label={app.translator.trans('talk.site.security.global_logout.heading')}>
          <span className="helpText">{app.translator.trans('talk.site.security.global_logout.help_text')}</span>
          <Button
            className="Button"
          ...
          >
            {app.translator.trans('talk.site.security.global_logout.log_out_button')}
          </Button>
        </FieldSet>
      );
    }
  ...
  developerTokensItems() {
    ...
    if (this.user!.id() === app.session.user!.id()) {
      items.add(
        ...
        >
          {app.translator.trans('talk.site.security.new_access_token_button')}
        </Button>
      );
    }
  ...
  sessionsItems() {
    ...
    if (this.user!.id() === app.session.user!.id()) {
      ...
      let terminateAllOthersButton = (
        ...
        >
          {app.translator.trans('talk.site.security.terminate_all_other_sessions')}
        </Button>
      );

      if (isDisabled) {
        terminateAllOthersButton = (
          <Tooltip text={app.translator.trans('talk.site.security.cannot_terminate_current_session')}>
  ...
  terminateAllOtherSessions() {
    if (!confirm(extractText(app.translator.trans('talk.site.security.terminate_all_other_sessions_confirmation')))) return;
    ...
    return app
      ...
      .then(() => {
        ...
        app.alerts.show({ type: 'success' }, app.translator.trans('talk.site.security.session_terminated', { count }));
      })
      .catch(() => {
        app.alerts.show({ type: 'error' }, app.translator.trans('talk.site.security.session_termination_failed'));
      })
      .finally(() => {
