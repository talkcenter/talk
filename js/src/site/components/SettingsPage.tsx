...
export default class SettingsPage<CustomAttrs extends IUserPageAttrs = IUserPageAttrs> extends UserPage<CustomAttrs> {
  ...
  oninit(vnode: Mithril.Vnode<CustomAttrs, this>) {
    ...
    app.setTitle(extractText(app.translator.trans('talk.site.settings.title')));
  }
  ...
  settingsItems() {
    ...
    ['account', 'notifications', 'privacy'].forEach((section, index) => {
      ...
      items.add(
        section,
        <FieldSet className={`Settings-${section}`} label={app.translator.trans(`talk.site.settings.${section}_heading`)}>
  ...
  accountItems() {
    ...
    items.add(
      ...
      <Button className="Button" onclick={() => app.modal.show(ChangePasswordModal)}>
        {app.translator.trans('talk.site.settings.change_password_button')}
      </Button>,
      100
    );
    ...
    items.add(
      ...
      <Button className="Button" onclick={() => app.modal.show(ChangeEmailModal)}>
        {app.translator.trans('talk.site.settings.change_email_button')}
      </Button>,
      90
    );
  ...
  privacyItems() {
    ...
    items.add(
      ...
      >
        {app.translator.trans('talk.site.settings.privacy_disclose_online_label')}
      </Switch>,
      100
    );
