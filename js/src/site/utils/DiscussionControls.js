...
export default {
        ...
        !app.session.user || discussion.canReply() ? (
          ...
          >
            {app.translator.trans(
              app.session.user ? 'talk.site.discussion_controls.reply_button' : 'talk.site.discussion_controls.log_in_to_reply_button'
            )}
          </Button>
        ) : (
          <Button
            icon="fas fa-reply"
            className="disabled"
            title={extractText(app.translator.trans('talk.site.discussion_controls.cannot_reply_text'))}
          >
            {app.translator.trans('talk.site.discussion_controls.cannot_reply_button')}
          </Button>
        )
      );
    }
    ...
    if (discussion.canRename()) {
      items.add(
        ...
        <Button icon="fas fa-pencil-alt" onclick={this.renameAction.bind(discussion)}>
          {app.translator.trans('talk.site.discussion_controls.rename_button')}
        </Button>
      );
    }
      ...
      if (discussion.canHide()) {
        items.add(
          ...
          <Button icon="far fa-trash-alt" onclick={this.hideAction.bind(discussion)}>
            {app.translator.trans('talk.site.discussion_controls.delete_button')}
          </Button>
        );
      }
    } else {
      if (discussion.canHide()) {
        items.add(
          ...
          <Button icon="fas fa-reply" onclick={this.restoreAction.bind(discussion)}>
            {app.translator.trans('talk.site.discussion_controls.restore_button')}
          </Button>
        );
      }
      ...
      if (discussion.canDelete()) {
        items.add(
          ...
          <Button icon="fas fa-times" onclick={this.deleteAction.bind(discussion)}>
            {app.translator.trans('talk.site.discussion_controls.delete_forever_button')}
          </Button>
        );
      }
    }
   ...
   */
  deleteAction() {
    if (confirm(extractText(app.translator.trans('talk.site.discussion_controls.delete_confirmation')))) {
