    ...
    view() {
      return (
        <Button className="Button Button--link" onclick={this.onclick.bind(this)} loading={this.loading} disabled={this.sent}>
          {this.sent
            ? [icon('fas fa-check'), ' ', app.translator.trans('talk.site.user_email_confirmation.sent_message')]
            : app.translator.trans('talk.site.user_email_confirmation.resend_button')}
        </Button>
      );
    }
  ...
  m.mount($('<div className="App-notices"/>').insertBefore('#content')[0], {
    view: () => (
      <ContainedAlert dismissible={false} controls={[<ResendButton />]} className="Alert--emailConfirmation">
        {app.translator.trans('talk.site.user_email_confirmation.alert_message', { email: <strong>{user.email()}</strong> })}
      </ContainedAlert>
    ),
  });
}
