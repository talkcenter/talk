...
export default {
      ...
      if (!post.isHidden()) {
        items.add(
          ...
          <Button icon="fas fa-pencil-alt" onclick={this.editAction.bind(post)}>
            {app.translator.trans('talk.site.post_controls.edit_button')}
          </Button>
        );
      }
    }
      ...
      if (post.canHide()) {
        items.add(
          ...
          <Button icon="far fa-trash-alt" onclick={this.hideAction.bind(post)}>
            {app.translator.trans('talk.site.post_controls.delete_button')}
          </Button>
        );
      }
    } else {
      if (post.contentType() === 'comment' && post.canHide()) {
        items.add(
          ...
          <Button icon="fas fa-reply" onclick={this.restoreAction.bind(post)}>
            {app.translator.trans('talk.site.post_controls.restore_button')}
          </Button>
        );
      }
      if (post.canDelete()) {
        items.add(
          ...
          <Button icon="fas fa-times" onclick={this.deleteAction.bind(post, context)}>
            {app.translator.trans('talk.site.post_controls.delete_forever_button')}
          </Button>
        );
      }
    }
   ...
   */
  hideAction() {
    if (!confirm(extractText(app.translator.trans('talk.site.post_controls.hide_confirmation')))) return;
  ...
  deleteAction(context) {
    if (!confirm(extractText(app.translator.trans('talk.site.post_controls.delete_confirmation')))) return;
