...
export default {
    ...
    if (user.canEdit() || user.canEditCredentials() || user.canEditGroups()) {
      items.add(
        ...
        <Button icon="fas fa-pencil-alt" onclick={this.editAction.bind(this, user)}>
          {app.translator.trans('talk.site.user_controls.edit_button')}
        </Button>
      );
    }
    ...
    if (user.id() !== '1' && user.canDelete()) {
      items.add(
        ...
        <Button icon="fas fa-times" onclick={this.deleteAction.bind(this, user)}>
          {app.translator.trans('talk.site.user_controls.delete_button')}
        </Button>
      );
    }
   ...
   */
  deleteAction(user) {
    if (!confirm(app.translator.trans('talk.site.user_controls.delete_confirmation'))) {
      return;
    }
  ...
  showDeletionAlert(user, type) {
    const message = {
      success: 'talk.site.user_controls.delete_success_message',
      error: 'talk.site.user_controls.delete_error_message',
    }[type];
