...
use Talk\Extension\Event\Enabling;
use Talk\Extension\Event\Uninstalled;
use Talk\Extension\Exception\CircularDependenciesException;
...
class ExtensionManager
{
    ...
    public function getExtensions()
    {
        if (is_null($this->extensions) && $this->filesystem->exists($this->paths->vendor.'/composer/installed.json')) {
            ...
            // We calculate and store a set of composer package names for all installed Talk extensions,
            // so we know what is and isn't a talk extension in `calculateDependencies`.
            ...
            foreach ($installed as $package) {
                ...
                if (Arr::get($package, 'type') === 'talk-extension') {
                    $composerJsonConfs[$packagePath] = $package;
                }

                if ($subextPaths = Arr::get($package, 'extra.talk-subextensions', [])) {
                    foreach ($subextPaths as $subExtPath) {
                        ...
                        if (Arr::get($conf, 'type') === 'talk-extension') {
    ...
    /**
     * Call on all enabled extensions to extend the Talk application.
     *
     ...
     *      'valid' points to an ordered array of \Talk\Extension\Extension
