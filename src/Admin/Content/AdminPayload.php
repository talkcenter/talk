...
use Talk\Foundation\ApplicationInfoProvider;
use Talk\Foundation\Config;
use Talk\Frontend\Document;
use Talk\Group\Permission;
use Talk\Settings\Event\Deserializing;
...
class AdminPayload
{
    ...
    public function __invoke(Document $document, Request $request)
    {
        ...
        $document->payload['displayNameDrivers'] = array_keys($this->container->make('talk.user.display_name.supported_drivers'));
        $document->payload['slugDrivers'] = array_map(function ($resourceDrivers) {
            return array_keys($resourceDrivers);
        }, $this->container->make('talk.http.slugDrivers'));
        ...
        /**
         * Used in the admin user list. Implemented as this as it matches the API in talk/statistics.
         * If talk/statistics ext is enabled, it will override this data with its own stats.
         *
