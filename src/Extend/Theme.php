...
class Theme implements ExtenderInterface
{
     ...
     * @param string|null $extensionId : If overriding an extension file, specify its ID, for example: `talk-tags`.
     * @return self
     */
     ...
     * ```php
     * (new Extend\Theme)
     *     ->addCustomLessFunction('is-talk', function (mixed $text) {
     *         return strtolower($text) === 'talk'
     *     }),
     * ```
     *
    ...
    {
        $container->extend('talk.frontend.custom_less_functions', function (array $customFunctions) {
            return array_merge($customFunctions, $this->customFunctions);
        });

        $container->extend('talk.less.custom_variables', function (array $lessVariables) {
            return array_merge($this->lessVariables, $lessVariables);
        });

        $container->extend('talk.assets.factory', function (callable $factory) {
