...
class ModelVisibility implements ExtenderInterface
{
     ...
     *                           This model must extend from \Talk\Database\AbstractModel,
     *                           and use \Talk\Database\ScopeVisibilityTrait.
     */
    public function __construct(string $modelClass)
    {
        ...
        if (class_exists($this->modelClass) && ! is_callable([$modelClass, 'registerVisibilityScoper'])) {
            throw new Exception("Model $modelClass cannot be visibility scoped as it does not use Talk\Database\ScopeVisibilityTrait.");
        }
    }
