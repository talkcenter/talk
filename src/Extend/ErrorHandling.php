...
class ErrorHandling implements ExtenderInterface
{
     ...
     * itself (if it implements {@see \Talk\Foundation\KnownError}), or
     ...
     * the exception implement our {@see \Talk\Foundation\KnownError}
     ...
     *
     * When Talk's default error handling is not enough for you, and the other
     ...
     * returns a {@see \Talk\Foundation\ErrorHandling\HandledError} instance.
     ...
     *
     * Reporters will be called whenever Talk encounters an exception that it
     ...
     * {@see \Talk\Foundation\ErrorHandling\Reporter} interface.
     *
    ...
    public function extend(Container $container, Extension $extension = null)
    {
        if (count($this->statuses)) {
            $container->extend('talk.error.statuses', function ($statuses) {
                return array_merge($statuses, $this->statuses);
            });
        }

        if (count($this->types)) {
            $container->extend('talk.error.classes', function ($types) {
                return array_merge($types, $this->types);
            });
        }

        if (count($this->handlers)) {
            $container->extend('talk.error.handlers', function ($handlers) {
