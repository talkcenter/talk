...
class LanguagePack implements ExtenderInterface, LifecycleInterface
{
    ...
    public function extend(Container $container, Extension $extension = null)
    {
        ...
        $locale = $extension->composerJsonAttribute('extra.talk-locale.code');
        $title = $extension->composerJsonAttribute('extra.talk-locale.title');

        if (! isset($locale, $title)) {
            throw new RuntimeException(
                'Language packs must define "extra.talk-locale.code" and "extra.talk-locale.title" in composer.json.'
            );
        }
    ...
    {
        $container->make('talk.locales')->clearCache();
    }
