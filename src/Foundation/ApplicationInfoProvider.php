...
class ApplicationInfoProvider
{
    ...
    public function getSchedulerStatus(): string
    {
        ...
        if (! $status) {
            return $this->translator->trans('talk.admin.dashboard.status.scheduler.never-run');
        }

        // If the schedule has not run in the last 5 minutes, mark it as inactive.
        return Carbon::parse($status) > Carbon::now()->subMinutes(5)
            ? $this->translator->trans('talk.admin.dashboard.status.scheduler.active')
            : $this->translator->trans('talk.admin.dashboard.status.scheduler.inactive');
    }
